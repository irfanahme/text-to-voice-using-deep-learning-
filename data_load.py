            ...
            for line in lines:
                try:
                    fname, _, text = line.strip().split("|")
                      ...
                      fname, text, _, _ = line.strip().split("|")
...
def get_batch():
    ...
    with tf.device('/cpu:0'):
        ...
        # Create Queues
        try:
            fpath, text_length, text = tf.train.slice_input_producer([fpaths, 
                                                                      text_lengths, 
                                                                      texts], 
                                                                     shuffle=True)
        except:
            fpath, text_length, text = tf.compat.v1.train.slice_input_producer([fpaths, 
                                                                      text_lengths, 
                                                                      texts], 
                                                                     shuffle=True)
        # Parse
