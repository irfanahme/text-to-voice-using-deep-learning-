    ...
    def __init__(self, num=1, mode="train"):
        ...
        if training:
            if num==1: # Text2Mel
                ...
                tf.compat.v1.summary.scalar('train/loss_mels', self.loss_mels)
                tf.compat.v1.summary.scalar('train/loss_bd1', self.loss_bd1)
                tf.compat.v1.summary.scalar('train/loss_att', self.loss_att)
                tf.compat.v1.summary.image('train/mel_gt', tf.expand_dims(tf.transpose(self.mels[:1], [0, 2, 1]), -1))
                tf.compat.v1.summary.image('train/mel_hat', tf.expand_dims(tf.transpose(self.Y[:1], [0, 2, 1]), -1))
            else: # SSRN
                # mag L1 loss
                ...
                tf.compat.v1.summary.scalar('train/loss_mags', self.loss_mags)
                tf.compat.v1.summary.scalar('train/loss_bd2', self.loss_bd2)
                tf.compat.v1.summary.image('train/mag_gt', tf.expand_dims(tf.transpose(self.mags[:1], [0, 2, 1]), -1))
                tf.compat.v1.summary.image('train/mag_hat', tf.expand_dims(tf.transpose(self.Z[:1], [0, 2, 1]), -1))
    ...
    with sv.managed_session() as sess:
        while 1:
            ...
            for _ in tqdm(range(g.num_batch), total=g.num_batch, ncols=70, leave=False, unit='b'):
                ...
                # Writ""e checkpoint files at every 1k steps
                #print("$$$$$$$$$$$$$$$$$$$$$$$$", gs)
                if gs % 1000 == 0:
